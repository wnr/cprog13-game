...
unsigned int game::rand(unsigned int max, bool inclusive) {
    if(max == 0 && inclusive) {
        throw std::invalid_argument("Cannot have max 0 and inclusive.");
    }
    ...
    return (unsigned int)((std::rand() % max) + (inclusive ? 1 : 0)); //Generates number between 1 and max inclusively.
}
